...
export const F = '-apple-system, BlinkMacSystemFont, "SF Pro Display", "SF Pro Text", system-ui, sans-serif';
...
export function ChooseState({ usage, error, reduced, onPro, onMax, onUltra, onStandard, onByok }: {
...
export function HeroCard({ title, price, period, icon: Icon, spec, accent, reduced, onClick }: {
...
export function TierCard({ title, price, period, icon: Icon, spec, accent, badge, badgeWarn, onClick }: {
  title:string; price:string; period:string; icon:React.ElementType;
...
export function SlimCard({ title, price, icon: Icon, spec, accent, onClick }: {
...
export function ByokRow({ onClick }: { onClick:()=>void }) {
  const [hov, setHov] = useState(false);
  const reduced = useReducedMotion() ?? false;
  return (
...
export function DoneState({ onDone }: { onDone?:()=>void }) {
  return (
    <div style={{padding:'52px 28px',display:'flex',flexDirection:'column',alignItems:'center',gap:'18px',textAlign:'center'}}>
